...
The bitline develop $\tbl$ is now the \textbf{sense-signal settling time},
derived per sensing mode from Upton (2024) (current-based readout,
App.~A; steady-state voltage sensing, App.~B). Both modes reduce to one
polynomial in the $\Nwl$ cells on the bitline: a linear \emph{signal-development}
term plus the shared quadratic BL wire self-RC $a_{\mathrm{quad}}=\tfrac12
r_{BL}c_{BL}$, all scaled by a settling factor $f_\Delta$ that carries the
sensing margin as a \emph{time} cost. For current sensing the development time
constant is $\tau_C = C_{BL}\,nU_t/I_{\mathrm{read}}$ (Eq.~A.12), giving
$a_{\mathrm{lin}}=c_{BL}\,nU_t/I_{\mathrm{read}}$ and $f_\Delta=-\ln(1-\Delta)$;
for voltage sensing the Elmore constant $\tau=\tfrac{V_{BL}}{V_{\mathrm{read}}}
(R_{\text{pull-up}}+R_{BL}/2)C_{BL}$ (Eq.~B.7) gives a pull-up-dominated
$a_{\mathrm{lin}}=v_{\mathrm{ratio}}R_{\text{pull-up}}c_{BL}$ and, from the
synchronous bound $T_{\mathrm{clk}}\ge-2\tau\ln(1-\Delta)$ (Eq.~B.8),
$f_\Delta=-2\ln(1-\Delta)$. The settling fraction $\Delta$ (e.g.\ $0.99$) is a
fixed reliability requirement, not a free variable; tightening it (Eq.~B.8)
costs time logarithmically. $\tsa$ retains the intrinsic device read/latch floor
(and restore, for destructive cells). Decoder volume tracks the array it drives;
sense-amp volume is a fixed footprint per amp --- the earlier Pelgrom area law
